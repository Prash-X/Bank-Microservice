...
public class CustomerController {
    ...
    public ResponseEntity<CustomerDetailsDto> fetchCustomerDetails(@RequestHeader("madhousebank-correlation-id") String correlationId,
                                                                    @RequestParam @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                                   String mobileNumber){
        logger.debug("Madhousebank-correlation-id found in CustomerController: {}", correlationId);
